	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(file != null) {
			...
			if(ext.equals("jpg") || ext.equals("png") || ext.equals("gif")) {
				String prev = new MemberDAO().getMember(memberID).getMemberProfile();
				...
				session.setAttribute("messageContent", "이미지 파일만 가능합니다.");
				response.sendRedirect("profileUpdate.jsp");
				return;
			}
		}
		new MemberDAO().profile(memberID, fileName);
		session.setAttribute("messageType", "성공 메시지");
		session.setAttribute("messageContent", "성공적으로 프로필을 수정했습니다.");
		response.sendRedirect("index.jsp");
		return;
	}
